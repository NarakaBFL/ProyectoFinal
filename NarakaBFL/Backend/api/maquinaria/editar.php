<?php

require_once __DIR__ . '/../../config/database.php';
require_once __DIR__ . '/../../config/session.php';
require_once __DIR__ . '/../../helpers/response.php';

if ($_SERVER['REQUEST_METHOD'] !== 'PUT') {
    responder(405, null, 'Método no permitido');
}

if (!isset($_SESSION['usuario_actual'])) {
    responder(401, null, 'No hay una sesión activa');
}

if ((int) $_SESSION['usuario_actual']['id_rol'] !== 2) {
    responder(403, null, 'No tienes permiso para realizar esta acción');
}

$datos = obtenerBody();

$idMaquinaria = (int) ($datos['id_maquinaria'] ?? 0);
$tipo = trim($datos['tipo'] ?? '');
$estado = trim($datos['estado'] ?? '');
$fechaUltimoMantenimiento = $datos['fecha_ultimo_mantenimiento'] ?? null;
$idCentro = (int) ($datos['id_centro'] ?? 0);

$estadosValidos = ['disponible', 'en_uso', 'mantenimiento', 'fuera_de_servicio'];

if ($idMaquinaria <= 0) {
    responder(400, null, 'ID de la maquinaria inválido');
} 

if ($tipo === '') {
    responder(400, null, 'El tipo de maquinaria es obligatorio');
}

if (!in_array($estado, $estadosValidos, true)) {
    responder(400, null, 'El estado ingresado no es válido');
}

if ($idCentro <= 0) {
    responder(400, null, 'El centro de acopio es obligatorio');
}

//Comprobar que el camión exista
$stmt = $pdo->prepare("
SELECT id_maquinaria
FROM maquinaria
WHERE id_maquinaria = ?");

$stmt->execute([$idMaquinaria]);

if (!$stmt->fetch()) {
    responder(404, null, 'La maquinaria no existe');
}

// Verificar que el centro de acopio exista
$stmt = $pdo->prepare("
SELECT id_centro
FROM centro_acopio
WHERE id_centro = ?");

$stmt->execute([$idCentro]);

if (!$stmt->fetch()) {
    responder(409, null, 'El centro de acopio no existe');
}

//Actualizar maquinaria
$stmt = $pdo->prepare("
UPDATE maquinaria
SET tipo = ?,
estado = ?,
fecha_ultimo_mantenimiento = ?,
id_centro = ?
WHERE id_maquinaria = ?");

$stmt->execute([
    $tipo,
    $estado,
    $fechaUltimoMantenimiento,
    $idCentro,
    $idMaquinaria
]);

responder(200, null, 'Maquinaria actualizada correctamente');

?>